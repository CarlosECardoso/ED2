...
public class Tree {
	  ...
	  public void buscalargura_ArvBin() {
		if (root == null) return;
	...
	public int quasecompleta(No node) {
		...
		while (!queue.isEmpty()) {
			...
			if (tempNode.esq != null) {
				...
				queue.add(tempNode.esq);
			} else flag = true;
	
			if (tempNode.dir != null) {
				if (flag == true) return 0;
				queue.add(tempNode.dir);
			} else flag = true;
		}
	...
	public int profundidade(int valor_no) {
		return profundidade(root, valor_no, 0);
	}
	
	private int profundidade(No node, int valor_no, int depth) {
		if (node == null) return -1;
		if (node.item == valor_no) return depth;
	
		int downlevel = profundidade(node.esq, valor_no, depth + 1);
		if (downlevel != -1) return downlevel;
	
		downlevel = profundidade(node.dir, valor_no, depth + 1);
		return downlevel;
	}
	 ...
	 * @return
	 */
	/*quando implementar no app usar codição if(arvorebusca(node))
	 * 											retornobusca=1;
	 * 										else 
	 * 											retornobusca=0;
	 */
	public int arvorebusca(No node) {
		if (node == null) return 1;
	
		if (node.esq != null && maxValue(node.esq) > node.item) return 0;
		if (node.dir != null && minValue(node.dir) < node.item) return 0;
		if (arvorebusca(node.esq)==0) {
			if ( arvorebusca(node.dir)==0){
				return 0;
			}
		}
